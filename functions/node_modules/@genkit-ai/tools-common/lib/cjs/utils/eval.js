...
exports.getEvalExtractors = exports.confirmLlmUse = exports.isEvaluator = exports.evaluatorName = exports.EVALUATOR_METADATA_KEY_IS_BILLED = exports.EVALUATOR_METADATA_KEY_DEFINITION = exports.EVALUATOR_METADATA_KEY_DISPLAY_NAME = exports.EVALUATOR_ACTION_PREFIX = void 0;
...
async function confirmLlmUse(evaluatorActions, force) {
    const isBilled = evaluatorActions.some((action) => action.metadata && action.metadata[exports.EVALUATOR_METADATA_KEY_IS_BILLED]);
    if (!isBilled) {
        return true;
    }
    if (force) {
        logger_1.logger.warn('For each example, the evaluation makes calls to APIs that may result in being charged.');
        return true;
    }
...
function getRootSpan(trace, shouldSucceed = true) {
    return Object.values(trace.spans).find((s) => s.attributes['genkit:type'] === 'flow' &&
        (shouldSucceed
            ? s.attributes['genkit:metadata:flow:state'] === 'done'
            : true));
}
const DEFAULT_INPUT_EXTRACTOR = (trace) => {
    const rootSpan = getRootSpan(trace, false);
...
async function getEvalExtractors(flowName) {
