...
export async function confirmLlmUse(evaluatorActions, force) {
    const isBilled = evaluatorActions.some((action) => action.metadata && action.metadata[EVALUATOR_METADATA_KEY_IS_BILLED]);
    if (!isBilled) {
        return true;
    }
    if (force) {
        logger.warn('For each example, the evaluation makes calls to APIs that may result in being charged.');
        return true;
    }
...
export async function getEvalExtractors(flowName) {
